...
void LevelFourScene::Load()
{

    // Print loading messages to console
    std::cout << "\n\033[33mLevel Four has Loaded!...\033[0m" << std::endl;
    ...
    mBarrier[0] = new Barrier(mRenderer, { 115, 475 });
    mBarrier[0]->SetDimensions({ 25, 25 });
    mBarrier[0]->SetXRange(115.f, 275.f, { 200.f, 0.0f });
    mBarrier[1] = new Barrier(mRenderer, { 275, 350 });
    mBarrier[1]->SetDimensions({ 25, 25 });
    mBarrier[1]->SetXRange(115.f, 275.f, { 200.f, 0.0f });
    mBarrier[2] = new Barrier(mRenderer, { 530, 500 });
    mBarrier[2]->SetDimensions({ 25, 25 });
    mBarrier[2]->SetXRange(530.f, 670.f, { 200.0f, 0.0f });
    mBarrier[3] = new Barrier(mRenderer, { 400, 120 });
    mBarrier[3]->SetDimensions({ 25, 25 });
    mBarrier[3]->SetYRange(120.f, 279.f, { 0.0f, 200.f });
    mBarrier[4] = new Barrier(mRenderer, { 310, 279 });
    mBarrier[4]->SetDimensions({ 25, 25 });
    mBarrier[4]->SetYRange(120.f, 279.f, { 0.0f, 300.f });
    mBarrier[5] = new Barrier(mRenderer, { 485, 279 });
    mBarrier[5]->SetDimensions({ 25, 25 });
    mBarrier[5]->SetYRange(120.f, 279.f, { 0.0f, 300.f });
    mBarrier[6] = new Barrier(mRenderer, { 485, 279 });
    mBarrier[6]->SetDimensions({ 25, 25 });
    mBarrier[6]->SetYRange(120.f, 279.f, { 0.0f, 300.f });
    mBarrier[7] = new Barrier(mRenderer, { 130, 250 });
    mBarrier[7]->SetDimensions({ 25, 25 });
    mBarrier[7]->SetXRange(130.f, 220.f, { 150.f, 0.f });
    mBarrier[8] = new Barrier(mRenderer, { 690, 350 });
    mBarrier[8]->SetDimensions({ 25, 25 });
    mBarrier[8]->SetXRange(530.f, 690.f, { 200.f, 0.0f });
    ...
    mPoints[0] = new Points(mRenderer, { 120, 410 });
    mPoints[0]->SetDimensions({ 30,30 });
    mPoints[0]->SetXPRange(120.f, 275.f, { 120.f ,0.f });
    mPoints[1] = new Points(mRenderer, { 530, 440 });
...
void LevelFourScene::Update(const InputState& input, double deltaTime)
{
    // Call base Update
   ...
   // Define a Vector2f variable called "normal"
    ...
    for (size_t i = 0; i < mBarrierCount; ++i)
    {
        ...
        else if (mIsSlowMotionActive)
        {
            ...
            mBarrier[i]->Update(input, deltaTime * 0.5);
        }
        else
        {
    ...
    for (size_t i = 0; i < mHoleCount; ++i)
    {
        ...
        if (pHole->DoesBodyIntersect(*mBall, normal, depth))
        {
            ...
            mNextScene = new LevelFiveScene(mRenderer, mViewport, mScreenWidth, mScreenHeight);
        }
    }
...
void LevelFourScene::Draw(double deltaTime)
{
    // Clear to dark green
