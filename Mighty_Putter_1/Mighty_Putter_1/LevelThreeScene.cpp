...
void LevelThreeScene::Load()
{
    // Print loading messages to console
    std::cout << "\n\033[33mLevel Three has Loaded!...\033[0m" << std::endl;
...
void LevelThreeScene::Update(const InputState& input, double deltaTime)
{
    // Call base Update
   ...
   // Define a Vector2f variable called "normal"
    ...
    for (size_t i = 0; i < mHoleCount; ++i)
    {
        ...
        if (pHole->DoesBodyIntersect(*mBall, normal, depth))
        {
            ...
            mNextScene = new LevelFourScene(mRenderer, mViewport, mScreenWidth, mScreenHeight);
        }
    }
...
void LevelThreeScene::Draw(double deltaTime)
{
    // Clear to dark green
