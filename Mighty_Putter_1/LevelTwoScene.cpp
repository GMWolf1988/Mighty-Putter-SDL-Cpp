...
void LevelTwoScene::Load()
{
    ...
    std::cout << "\n\033[33mLevel Two has Loaded!...\033[0m" << std::endl;
    ...
    mMulti = new MultiplyPoints(mRenderer, { 530, 250 });

    // Create wall pieces
    mWallCount = 10;
    mWalls = (Wall**)malloc(mWallCount * sizeof(Wall*));
    mWalls[0] = new Wall(mRenderer, { 84, 471 }, 90);
    mWalls[1] = new Wall(mRenderer, { 84, 309 }, 90);
    mWalls[1]->SetDimensions({ 130, 32 });
    mWalls[2] = new Wall(mRenderer, { 168, 228 });
    mWalls[3] = new Wall(mRenderer, { 368, 228 });
    mWalls[3]->SetDimensions({ 200, 32 });
    mWalls[4] = new Wall(mRenderer, { 484, 183 }, 90);
    mWalls[4]->SetDimensions({ 125, 32 });
    mWalls[5] = new Wall(mRenderer, { 172, 587 });
    mWalls[5]->SetDimensions({ 208,32 });
    mWalls[6] = new Wall(mRenderer, { 572, 105 });
    mWalls[6]->SetDimensions({ 208,32 });
    mWalls[7] = new Wall(mRenderer, { 475, 410 });
    mWalls[7]->SetDimensions({ 400,32 });
    mWalls[8] = new Wall(mRenderer, { 292, 500 }, 90);
    mWalls[9] = new Wall(mRenderer, { 691, 258 }, 90);
    mWalls[9]->SetDimensions({ 337,32 }); 
    ...
    mFloorPositions[0] = { 100, 395 };
    mFloorDimensions[0] = { 175, 175 };
    mFloorPositions[1] = { 100, 245 };
    mFloorDimensions[1] = { 400, 150 };
    mFloorPositions[2] = { 500, 120};
    mFloorDimensions[2] = { 175, 275 };

    // Create barriers
    mBarrierCount = 4;
    mBarrier = (Barrier**)malloc(mBarrierCount * sizeof(Barrier*));
    mBarrier[0] = new Barrier(mRenderer, { 130, 400 });
    mBarrier[0]->SetDimensions({ 25, 25 });
    mBarrier[0]->SetXRange(130.f, 250.f, { 200.0f, 0.0f });
    mBarrier[1] = new Barrier(mRenderer, { 400, 265 });
    mBarrier[1]->SetDimensions({ 25, 25 });
    mBarrier[2] = new Barrier(mRenderer, { 400, 290 });
    mBarrier[2]->SetDimensions({ 25, 25 });
    mBarrier[2]->SetYRange(290.f, 370.f, { 0.0f, 200.0f });
    mBarrier[3] = new Barrier(mRenderer, { 530, 250 });
    mBarrier[3]->SetDimensions({ 25, 25 });
    mBarrier[3]->SetXRange(530.f, 650.f, { 200.0f, 0.0f });

    // PowerUp setup
    mPowerUpCount = 1;
    mPowerUps = (PowerUps**)malloc(mPowerUpCount * sizeof(PowerUps*));
    mPowerUps[0] = new PowerUps(mRenderer, { 105, 275});
    mPowerUps[0]->SetDimensions({ 30, 30 });
    mPowerUps[0]->SetXPRange(105.f, 250.f, {200.f, 0.f});
   ...
   // Define a Vector2f variable called "normal"
    ...
    for (size_t i = 0; i < mHoleCount; ++i)
    {
        ...
        if (pHole->DoesBodyIntersect(*mBall, normal, depth))
        {
            ...
            mNextScene = new LevelThreeScene(mRenderer, mViewport, mScreenWidth, mScreenHeight);
        }
    }
...
void LevelTwoScene::Draw(double deltaTime)
{
    // Clear to dark green
